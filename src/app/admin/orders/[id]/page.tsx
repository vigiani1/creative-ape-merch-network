...
export default async function AdminOrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (itemError || eventError || ledgerError) throw new Error("Unable to load order details.");
    ...
    <div className="mx-auto grid max-w-6xl gap-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_.6fr]">
        ...
        <div className="grid gap-6">
          <section className="rounded-2xl border border-black/10 bg-white p-6">
            <p className="text-sm font-semibold text-black/45">Customer</p>
            <h2 className="mt-1 text-xl font-black">{order.customer_name || "Guest customer"}</h2>
            <p className="mt-2 text-sm text-black/60">{order.customer_email || "No email"}</p>
